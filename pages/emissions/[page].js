...
const emmission = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:1337/api/emissions?populate=*&pagination[pageSize]=9&pagination[page]=${router.query.page}`
      )
      .then((response) => {
      ...
      >
        ...
        {loading && (
          <div className="flex flex-col items-center w-full gap-10">
            ...
            <Pagination route='/emissions/' current={router.query.page} pages={shows.meta.pagination.pageCount} />
          </div>
        )}
      </div>
    </div>
